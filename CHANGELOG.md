...
- **Plan mode is re-enabled on SDK 17**, ported to the `xd://propose` device model. SDK 17 removed `setStandingResolveHandler` and inverted the control flow: `preparePlanForReview` now returns immediately instead of blocking inside a standing resolve handler, so the bridge instead watches the session's `tool_execution_end` stream for a `write` targeting `xd://propose` and runs approval **detached** from that event chain — awaiting it inline would stall every other event until execution finishes (SDK issue #7684). The in-flight turn is silently aborted first (`markPlanInternalAbortPending`/`clearPlanInternalAbortPending` around `session.abort()`) so the model doesn't re-propose in a loop while the card is up. Approved plans are no longer renamed — SDK 17 keeps a plan at the `local://` path the agent chose, so the **title-rename input is gone from the approval card**; it now shows the derived title read-only and the card is strictly approve / reject / edit-content. Restored tools on approval force-include `read` (execution now requires reading the durable plan file). The plan-mode-bridge test suite is fully unskipped (was `describe.skip`ped for the disable).
