...
/**
 * Inline plan-approval card (T-105 — Slice C, ported to SDK 17's
 * `xd://propose` model).
 *
 * Renders when the active session has `pendingPlanApproval` set — i.e. the
 * agent submitted a plan by writing its title to `xd://propose` while plan
 * mode was active and the deck bridge is awaiting the user's decision.
 *
 * Three actions:
 *   - Reject: `respondToPlanApproval({ approved: false })`. Server exits
 *     plan mode and surfaces a clear rejection to the agent. No file change.
 *   - Approve: `respondToPlanApproval({ approved: true })`. Server queues
 *     the plan-approved execution prompt as a followUp turn. The plan file
 *     is never renamed in SDK 17 — `local://` links to it stay valid.
 *   - Edit & approve: includes `editedContent` so the bridge writes the
 *     replacement to the plan file (same path) before approving.
 *
